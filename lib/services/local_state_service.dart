...
class LocalStateService {
  ...
  static Future<Map<String, dynamic>?> loadCart(String scope) async {
    final prefs = await SharedPreferences.getInstance();
    final key = _cartKey(scope);
    final raw = prefs.getString(key);
    if (raw == null || raw.isEmpty) return null;
    final decoded = jsonDecode(raw);
    if (decoded is! Map<String, dynamic>) return null;
    if ((decoded['schema_version'] as num?)?.toInt() != _schemaVersion) {
  ...
  static Order orderFromMap(Map<String, dynamic> map) {
    return Order(
      ...
      items: (map['items'] as List? ?? const [])
          .map(
            (entry) => cartItemFromMap(Map<String, dynamic>.from(entry as Map)),
          )
          .toList(),
      ...
      placedAt:
          ...
          DateTime.now(),
      estimatedDelivery:
          DateTime.tryParse((map['estimated_delivery'] ?? '').toString()) ??
          DateTime.now(),
      ...
      deliveryVerificationCode: (map['delivery_verification_code'] ?? '')
          .toString(),
    );
  }
