          ...
          body: ListView(
            ...
            children: [
              _AnalyticsCard(
                ...
                subtitle:
                    'Held revenue is waiting for completed delivery confirmation. Released revenue is ready after successful handoff.',
                child: Row(
                  children: [
                    Expanded(
                      child: _MetricPill(
                        label: 'Held',
                        value: 'Rs ${earnings.held.toInt()}',
                    ...
                    Expanded(
                      child: _MetricPill(
                        label: 'Released',
                        value: 'Rs ${earnings.released.toInt()}',
              ...
              _AnalyticsCard(
                ...
                subtitle:
                    'These figures come from the shared order lifecycle and inventory reservation logic.',
                child: Column(
                  children: [
                    ...
                    _summaryRow(
                      'Avg basket value',
                      'Rs ${orderService.averageOrderValueForStore(storeId).toInt()}',
                    ),
                    _summaryRow(
                      'Reserved stock buckets',
                    ...
                    _summaryRow(
                      'Service radius',
                      '${orderService.radiusForStore(storeId).toStringAsFixed(1)} km',
                    ),
                  ],
                ),
              ),
              ...
              _AnalyticsCard(
                ...
                subtitle:
                    'Use this to watch how the store is moving through placed, preparation, handoff, and delivered states.',
              ...
              _AnalyticsCard(
                ...
                subtitle:
                    'Reservation locks are created when orders are placed and released or consumed as fulfillment progresses.',
                child: activeOrders.isEmpty
                    ...
                    : Column(
                        children: activeOrders.take(6).map((order) {
                          final reservations = orderService
                              .inventoryReservationsForOrder(order.id);
                          return Padding(
                                ...
                                Text(
                                  reservations.isEmpty
                                      ...
                                      : reservations
                                            .map(
                                              (reservation) =>
                                                  '${reservation['reserved_quantity']}x ${(reservation['product_id'] ?? '').toString()} • ${(reservation['reservation_status'] ?? '').toString()}',
                                            )
                                            .join('\n'),
                                  style: const TextStyle(
                                    color: AppTheme.textMedium,
