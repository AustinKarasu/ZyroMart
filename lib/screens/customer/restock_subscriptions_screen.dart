...
class _RestockSubscriptionsScreenState
    ...
    try {
      ...
      if (_looksLikeMissingRestockTable(error)) {
        ...
        _errorMessage = _friendlyError(
          error,
          fallback:
              'Could not load live restock subscriptions right now. Please try again.',
        );
      }
    }
  ...
  Future<void> _togglePlan(Product product, bool enabled) async {
    ...
    try {
      final savedLive = await _savePlan(updated);
      if (savedLive) {
        _showMessage(
          enabled
        ...
        _showMessage(
          enabled
              ? 'Saved backup plan for ${product.name}. Live table is not available yet.'
              : 'Paused in backup plan for ${product.name}. Live table is not available yet.',
        );
      }
    } catch (error) {
      if (!mounted) return;
  ...
  Future<void> _updatePlan(
    ...
    try {
      ...
      _showMessage(
        savedLive
            ? 'Restock plan updated for ${product.name}.'
            : 'Saved backup plan for ${product.name}. Live table is not available yet.',
      );
    } catch (error) {
      if (!mounted) return;
