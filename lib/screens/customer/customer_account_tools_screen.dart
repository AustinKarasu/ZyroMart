...
class _WishlistScreenState extends State<WishlistScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final savedProducts = catalog.products
        ...
        .toList();
    final suggestions = catalog.products.where((product) {
      if (query.isEmpty) return true;
      return product.name.toLowerCase().contains(query) ||
          product.unit.toLowerCase().contains(query);
    }).take(24).toList();
              ...
              children: [
                TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: 'Search wishlist and all products',
                ...
                _SectionShell(
                  ...
                  subtitle: 'Every saved item stays account-scoped and syncs through your backend profile.',
                  child: savedProducts.isEmpty
                      ? const Padding(
                          padding: EdgeInsets.only(top: 8),
                          child: Text(
                            'No wishlist items match this search yet. Save products from home, search, or the suggestion shelf below.',
                            style: TextStyle(
                ...
                _SectionShell(
                  ...
                  subtitle: 'Search across the full catalog and pin items without leaving this page.',
                  child: Column(
...
class _PaymentMethodsScreenState extends State<PaymentMethodsScreen> {
  ...
  Future<void> _load() async {
    ...
    if (data.isNotEmpty) {
      ...
      _cardLastFourController.text = (data['card_last_four'] ?? '').toString();
    ...
    try {
      await _AccountStateRepository.savePaymentSettings(
        widget.userId,
        {
          ...
          'card_last_four': _cardLastFourController.text.trim(),
          'gateway': PaymentGatewayService.isRazorpayReady
              ? 'razorpay_ready'
              : 'pending_gateway_setup',
        },
      );
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        ...
        child: ListView(
          ...
          children: [
            _SectionShell(
              ...
              subtitle: 'Save the payment path you actually want the storefront to prioritize at checkout.',
              child: Column(
                children: [
                  Container(
                    padding: const EdgeInsets.all(16),
                    ...
                    child: Row(
                      children: [
                        const Icon(Icons.shield_outlined, color: Color(0xFF1D8C3A)),
                  ...
                  TextFormField(
                    ...
                    decoration: const InputDecoration(
                      ...
                      hintText: 'Optional for saved billing references',
                    ),
                    validator: (value) {
                      if (value == null || value.trim().isEmpty) return null;
                      if (value.trim().length != 4) return 'Enter exactly 4 digits';
                      return null;
                    },
                  ),
                  const SizedBox(height: 8),
                  SwitchListTile.adaptive(
                    ...
                    subtitle: const Text('Keep COD available when nearby stores support it.'),
...
class _WishlistProductTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListTile(
      ...
      subtitle: Text('Rs ${product.price.toStringAsFixed(0)} â€¢ ${product.unit}'),
      trailing: IconButton(
        onPressed: onToggle,
...
class _AccountStateRepository {
  ...
  static Future<void> saveAddresses(String userId, List<_SavedAddress> addresses) async {
    _requireLiveBackend('save addresses');
    final prefs = await SharedPreferences.getInstance();
    await prefs.setStringList(
  ...
  static Future<void> saveWishlist(String userId, List<String> ids) async {
    _requireLiveBackend('save wishlist');
    final prefs = await SharedPreferences.getInstance();
  ...
  static Future<void> savePaymentSettings(String userId, Map<String, dynamic> settings) async {
    _requireLiveBackend('save payment settings');
    final prefs = await SharedPreferences.getInstance();
  ...
  static Future<void> saveGstProfile(String userId, Map<String, dynamic> profile) async {
    _requireLiveBackend('save GST details');
    final prefs = await SharedPreferences.getInstance();
  ...
  static Future<void> saveGiftBalance(String userId, double balance) async {
    _requireLiveBackend('update gift balance');
    final prefs = await SharedPreferences.getInstance();
  ...
  static void _requireLiveBackend(String action) {
    if (!SupabaseService.isInitialized) {
      throw StateError(
        'Live backend is required to . ',
      );
    }
  }
}
